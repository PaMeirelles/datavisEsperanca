...
try:
    df_nodes = pd.read_csv("./all_nodes.csv")
...
print("Education", df_nodes["education"].dropna().unique())
...
node_data = df_nodes.groupby('id').first().reset_index()
...
# --- Define Edges ---
# Create edges by loading them from 'all_edges.csv'
try:
    df_edges = pd.read_csv("./all_edges.csv")
    for index, row in df_edges.iterrows():
        G.add_edge(row['id1'], row['id2'])
except FileNotFoundError:
    print("Warning: 'all_edges.csv' not found. The graph will be created with nodes but no edges.")
except KeyError:
    print("Error: 'all_edges.csv' must contain 'id1' and 'id2' columns.")
    exit()
